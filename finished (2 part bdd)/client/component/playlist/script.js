...
/* on charge le template du composant Playlist */
let template = await loadTemplate('./component/playlist/template.html');

/* on crée un objet Playlist vide qui va symboliser notre composant */
let Playlist = {}; 
...
/*  Playlist.render
    ...
    @param data: array, un tableau d'un ou plusieurs objets javascript, chacun décrivant les données d'un Playlist.
...
Playlist.render = async function(selector, data){
    // on formate un composant Playlist pour chaque objet du tableau
    let html = '';
    for(let obj of data){
        html += Playlist.format(obj); // on concatène (mettre bout à bout) les HTML des composants formatés  
    }
...
// on exporte le composant Playlist pour pouvoir l'utiliser ailleurs dans un autre modules JS
export {Playlist}

// Note : seul Playlist est exporté (et donc Playlist.render et Playlist.format)
