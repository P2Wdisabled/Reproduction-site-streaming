...
*/
Film.comments = async function(idfilm){
    let response = await fetch("../server/script.php?action=getCommentaires&idmovie="+idfilm);
    let data = await response.json();
    let comments = await Commentaire.render(data);
    return comments
}
Film.format = async function(obj) {
    let html = template;
    ...
    let comments = await Film.comments(obj.id_film);
    if (comments == "<li><p>null</p> - <p>[null]</p> : <p>null</p> | <p>null</p></li>") {
        html = html.replace('{{listCommentaires}}', "")
    } else {
        html = html.replace('{{listCommentaires}}', comments)
    }
    return html;
}
...
Film.render = async function(selector, data){
    ...
    let html = '';
    for(let obj of data){
        html += await Film.format(obj); // on concatène (mettre bout à bout) les HTML des composants formatés  
    }
