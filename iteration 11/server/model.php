...
function getCommentaires($id){
    $cnx = new PDO("mysql:host=localhost;dbname=potevin1", "potevin1", "potevin1");
    $answer = $cnx->query("select nom, date, commentaire, note from Commentaires JOIN Notes ON Commentaires.id_film = Notes.id_film JOIN Comptes ON Comptes.id_utilisateur = Notes.id_profil where Commentaires.id_film='$id'"); 
    $res = $answer->fetchAll(PDO::FETCH_OBJ);
    return $res;
}
