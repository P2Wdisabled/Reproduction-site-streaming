...
/* on charge le template du composant Films */
let template = await loadTemplate('./component/films/template.html');

/* on crée un objet Films vide qui va symboliser notre composant */
let Films = {}; 
...
Films.format =  function(obj) {
    let html = template;
    html = html.replace('{{idmovie}}', obj.id_film);
...
/*  Films.render
    ...
    @param data: array, un tableau d'un ou plusieurs objets javascript, chacun décrivant les données d'un Films.
...
Films.render = async function(selector, data){
    // on formate un composant Films pour chaque objet du tableau
    let html = '';
    for(let obj of data){
        html += Films.format(obj); // on concatène (mettre bout à bout) les HTML des composants formatés  
    }
...
// on exporte le composant Films pour pouvoir l'utiliser ailleurs dans un autre modules JS
export {Films}

// Note : seul Films est exporté (et donc Films.render et Films.format)
